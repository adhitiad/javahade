...
def generate_recovery_codes_view(request):
    ...
    request.session['new_recovery_codes'] = codes
    messages.success(request, "12 Kode Pemulihan baru berhasil dibuat. Kode lama Anda telah hangus.")
    return redirect("booking:recovery_codes")
...
def manage_host_rates_view(request):
        ...
        if duration_type and price:
            try:
                rate, created = HostBookingRate.objects.get_or_create(
                    ...
                    defaults={'price': price, 'currency': currency, 'is_active': is_active}
                )
                if not created:
                    rate.price = price
    ...
    return render(request, "booking/host_rates.html", {
        "rates": my_rates,
...
def book_host_view(request, username):
    ...
    if request.user.role != User.Role.HOST:
        ...
        if not has_kyc:
            messages.error(request, "Keamanan adalah prioritas. Anda diwajibkan untuk Verifikasi KTP (KYC) terlebih dahulu sebelum dapat mem-booking sesi privat dengan Host.")
    ...
    rates = HostBookingRate.objects.filter(host=host_user, is_active=True).order_by("price")
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    if payment_method in ['bnb', 'usdt']:
                        ...
                        if pay_currency == "IDR" and request.user.balance_idr < final_price:
                            raise ValueError(f"Saldo IDR tidak cukup. Anda butuh IDR {final_price:.2f}.")
                        elif pay_currency == "USD" and request.user.balance_usd < final_price:
                            raise ValueError(f"Saldo USD tidak cukup. Anda butuh USD {final_price:.2f}.")
                        elif pay_currency == "SGD" and request.user.balance_sgd < final_price:
                            raise ValueError(f"Saldo SGD tidak cukup. Anda butuh SGD {final_price:.2f}.")
                        elif pay_currency == "MYR" and request.user.balance_myr < final_price:
                            raise ValueError(f"Saldo MYR tidak cukup. Anda butuh MYR {final_price:.2f}.")
                        elif pay_currency == "CNY" and request.user.balance_cny < final_price:
                            raise ValueError(f"Saldo CNY tidak cukup. Anda butuh CNY {final_price:.2f}.")
                ...
                if booking_status == HostBooking.Status.PENDING_PAYMENT:
                    messages.success(request, f"Request booking berhasil dibuat. Silakan tunggu admin memverifikasi TXID Crypto Anda.")
                else:
                    messages.success(request, f"Berhasil mengirim request booking (Lunas) ke {host_user.username}!")
    ...
    from django.conf import settings
    return render(request, "booking/host_booking_form.html", {
        "host": host_user,
        "rates": rates,
...
def manage_host_bookings_view(request):
    ...
    return render(request, "booking/host_bookings_list.html", {
        "bookings": bookings
    })
...
def host_booking_detail_view(request, booking_id):
    ...
    client_kyc = KYCDocument.objects.filter(user=booking.user, status=KYCDocument.Status.APPROVED).order_by('-submitted_at').first()
    ...
    if client_kyc and client_kyc.birth_date:
        ...
        client_age = today.year - bday.year - ((today.month, today.day) < (bday.month, bday.day))
        
    # Mutual Rating Data (Host melihat rating Klien)
    client_avg_rating = booking.user.get_avg_rating_as_user()
    has_accepted_booking = HostBooking.objects.filter(host=request.user, status__in=[HostBooking.Status.CONFIRMED, HostBooking.Status.COMPLETED]).exists()
        
    return render(request, "booking/host_booking_detail.html", {
        "booking": booking,
...
def host_booking_action_view(request, booking_id):
    ...
    if request.method == "POST":
        ...
        if action == "accept" and booking.status == HostBooking.Status.PENDING:
            ...
            messages.success(request, f"Anda MENERIMA pesanan dari {booking.user.username}.")
            ...
            booking.save()
            messages.warning(request, f"Anda MENOLAK pesanan dari {booking.user.username}.")
            ...
            Notification.objects.create(
                ...
                body=f"Mohon maaf, {request.user.username} MENOLAK pesanan privat Anda.",
            )
...
def private_call_view(request, booking_id):
    ...
    booking = get_object_or_404(
        HostBooking, 
        Q(id=booking_id) & (Q(host=request.user) | Q(user=request.user))
    )
    
    if booking.status != HostBooking.Status.CONFIRMED:
        messages.error(request, "Sesi private call hanya bisa dimulai setelah booking dikonfirmasi.")
        return redirect("booking:dashboard")
        
    is_host = (request.user == booking.host)
    
    return render(request, "booking/private_call.html", {
        "booking": booking,
        "is_host": is_host
    })
...
def host_booking_noshow_view(request, booking_id):
    ...
    messages.success(request, "Pemesanan dibatalkan karena Klien tidak hadir. Dana kompensasi telah masuk ke dompet Anda.")
...
def client_report_fake_location_view(request, booking_id):
    ...
    with transaction.atomic():
        ...
        if host.fake_location_strikes == 1:
            ...
            messages.success(request, f"Laporan diterima. Host @{host.username} telah dikenakan penalti larangan booking selama 127 hari.")
        
        elif host.fake_location_strikes >= 2:
            # Second strike: Permanent ban and 20% auto-withdraw
            host.is_active = False # Banned
            host.booking_banned_until = timezone.now() + timedelta(days=3650) # 10 years
            ...
            messages.success(request, f"Laporan diterima. Host @{host.username} telah melakukan pelanggaran ke-2 dan akunnya diblokir permanen.")
...
def submit_rating_view(request, booking_id):
    ...
    if booking.status != HostBooking.Status.COMPLETED:
        messages.error(request, "Rating hanya dapat diberikan setelah pesanan berstatus Selesai (COMPLETED).")
    ...
    if is_client:
        if rating_obj.user_rating_of_host is not None:
            ...
            rating_obj.save()
            messages.success(request, "Rating berhasil dikirim! Terima kasih atas partisipasi Anda.")
    elif is_host:
...
def client_cancel_host_booking_view(request, booking_id):
    ...
    if booking.status != HostBooking.Status.CONFIRMED:
        messages.error(request, "Hanya pesanan berstatus CONFIRMED yang dapat dibatalkan dengan skema ini.")
        return redirect('booking:my_bookings')
    ...
    messages.success(request, f"Pesanan dibatalkan. Dana sebesar 85% (IDR {user_refund}) telah dikembalikan ke dompet Anda.")
    return redirect('booking:my_bookings')
...
def client_reschedule_host_booking_view(request, booking_id):
    ...
    if booking.reschedule_count >= 2:
        messages.error(request, "Batas maksimal Ubah Jadwal (2 kali) telah tercapai untuk pesanan ini.")
        return redirect('booking:my_bookings')
    ...
    try:
        ...
        new_start_datetime = make_aware(datetime.strptime(new_datetime_str, "%Y-%m-%d %H:%M"))
    except ValueError:
    ...
    with transaction.atomic():
        if reschedule_fee > 0 and request.user.balance_idr < reschedule_fee:
            messages.error(request, f"Saldo IDR tidak cukup untuk biaya Reschedule sebesar IDR {reschedule_fee}.")
            return redirect('booking:my_bookings')
