...
export function PostCard({ post, index, onLike, onReport }: PostCardProps) {
  ...
  const creatorUsername = typeof creator?.user === 'string' ? creator.user : creator?.user?.username ?? "unknown";
    ...
    <Card className="glass rounded-3xl border border-white/5 overflow-hidden mb-6 animate-fade-in shadow-[0_0_20px_rgba(0,0,0,0.3)] bg-gray-950/40 gap-0 py-0 hover:shadow-md transition-shadow">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              @{creatorUsername} • {timeSince(post.created_at)}
            </p>
          </div>
        </div>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button className="bg-transparent hover:text-white transition-colors p-1">
        ...
        <>
          {post.is_premium ? (
            <div className="relative w-full aspect-square bg-zinc-900 border-y border-white/5 flex flex-col items-center justify-center">
              <div className="absolute inset-0 bg-black/60 backdrop-blur-xl z-10 flex flex-col items-center justify-center p-6 text-center">
                ...
                <p className="text-sm text-gray-400 mb-6">
                  Berlangganan untuk membuka foto dan video rahasia.
                </p>
                <Button
                  className="px-8 py-3 bg-white hover:bg-gray-200 text-black font-bold rounded-full border-none"
                  onClick={() => router.push(`/u/${creatorUsername}`)}
                >
                  Subscribe Now
                </Button>
              </div>
            ...
            <div className="relative aspect-[4/3] bg-muted border-y border-white/5 overflow-hidden flex justify-center bg-black/50">
              <div
                className={`absolute inset-0 bg-gradient-to-br ${getPlaceholderGradient(index)} opacity-30`}
              />
      ...
      <div className="p-4 flex justify-between items-center bg-white/[0.02] border-t border-white/5">
        <div className="flex gap-4">
          <button
            className={`flex items-center gap-2 transition-all transform active:scale-90 ${liked ? "text-pink-500" : "text-gray-400 hover:text-pink-500"}`}
            onClick={handleLike}
          >
            <span className={`text-xl ${liked ? "scale-110" : ""}`}>
              {liked ? "❤️" : "🤍"}
            </span>
            <span className={`font-medium ${liked ? "text-pink-400" : ""}`}>
          ...
          <button
            ...
            onClick={() => alert(`Kirim tip untuk @${creatorUsername}!`)}
          >
            <span className="text-xl">💰</span>
        ...
        <button
          ...
          onClick={() => alert("Disimpan ke Bookmark!")}
        >
          <span className="text-xl">🔖</span>
        </button>
      </div>
        ...
        <div className="p-4 border-t border-white/5 bg-zinc-950/60 space-y-4">
          ...
          <form onSubmit={handleSendComment} className="flex gap-2 pt-2">
            ...
            <Button
              ...
              size="icon"
              className="rounded-xl size-10 shrink-0 bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-400 hover:to-pink-500"
            >
              {isSubmittingComment ? (
